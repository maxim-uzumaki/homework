...
const number = prompt(
	"Введите число, чтобы проверить, если оно четное или нечетное",
);
...
const age = prompt("Введите свой возраст, чтобы получить скидку");
...
if (weight > 0) {
	baseValue = weight < 1 ? 5 : weight >= 1 && weight <= 5 ? 10 : 15;
} else {
	alert("Некорректный вес посылки.");
}

const deliveryType = prompt(
	"Введите тип посылки(Стандарт, Экспресс, Премиум).",
);
let deliveryCoeff;
...
if (baseValue && deliveryCoeff) {
	const result = baseValue * deliveryCoeff;
	alert(`Итоговая стоимость доставки: ${result}$`);
} else {
	alert(`Итоговая стоимость не может быть рассчитана`);
}
